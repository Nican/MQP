...
Define the state space as $x={\theta, \omega}$, and the action space to be $u={\tau}$. We let pointing up, $\theta = 0$, as the pendulum to be pointing up, and we are optimizing the pendulum to reach $\theta = 0$, with zero velocity, $\omega = 0$, using the minimum torque necessary. We define the cost function be to $C$:

\begin{equation} 
C(x,u) = \omega^2 + 0.5 \cdot \omega^2 + \tau^2
\end{equation}

And we want to minimize the total cost function, C, given by the integral: 

\begin{equation} 
C = \int L(x,u) dt %TODO: WHERE DID L COME FROM?!
\end{equation}
...
And if $V'(x)<C(x,u) + \omega V(x_{k+1}(U(x))), we let V'(x)$ and $U(x)={\tau'}$.
...
Until 2001 developers could only use shaders that were pre-programmed into the hardware, and then Direct3D 8 (http://en.wikipedia.org/wiki/Direct3D) gave developers the option to program custom shaders that run in the hardware.

The main drive for GPU development was for computer graphics, but a technique known as fragment processing (?) was used to perform other types of computation. Fragment processing involves data processing and storage through image textures.
...
The gradratic programming is solved using a re-written version of QuadProg++[http://quadprog.sourceforge.net/], but adapted to use Eigen. 
